...
import { useState, useEffect } from 'react'; // Added for frontend integration
import { logger } from '../../lib/utils/logger.js'; // Added for global logger

// Enhanced Apps Page with API integration
const AppsPage = () => {
  const [apps, setApps] = useState([]);
  const [loading, setLoading] = useState(true);
  const [formData, setFormData] = useState({ platform: '', appName: '', clientId: '', clientSecret: '' });
  const [error, setError] = useState(null);

  // Check auth token (from oauth2-server)
  ...
  const loadApps = async () => {
    try {
      ...
      if (res.ok) {
        ...
        window.location.href = '/auth/login'; // Redirect to re-auth
      }
    } catch (err) {
      setError('Failed to load apps');
      logger.error(err); // Assume global logger
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('/api/save-app', {
        method: 'POST',
        headers: { 
      ...
      if (res.ok) {
        ...
        setFormData({ platform: '', appName: '', clientId: '', clientSecret: '' });
      } else {
  ...
  const handleDelete = async (platform) => {
    try {
      await fetch(`/api/delete-app?platform=${platform}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      loadApps();
  ...
  return `
    <div class="apps-page">
      ...
      <form onsubmit="handleSubmit(event)">
        <select value="${formData.platform}" onchange="setFormData({...formData, platform: this.value})">
          <!-- Options from /api/platforms -->
        </select>
        <input placeholder="App Name" value="${formData.appName}" onchange="setFormData({...formData, appName: this.value})" />
        <input placeholder="Client ID" value="${formData.clientId}" onchange="setFormData({...formData, clientId: this.value})" />
        <input placeholder="Client Secret" type="password" value="${formData.clientSecret}" onchange="setFormData({...formData, clientSecret: this.value})" />
      ...
      <ul>
        ${apps.map(app => `
          <li>
            ...
            <button onclick="handleDelete('${app.platform}')">Delete</button>
          </li>
        `).join('')}
      </ul>
    </div>
    <script>
      // Global functions for form (since server-rendered, expose handleSubmit etc.)
      window.handleSubmit = handleSubmit;
      window.setFormData = (newData) => { formData = newData; /* Update form */ };
      window.handleDelete = handleDelete;
    </script>
  `;
};

export default AppsPage; // For getModernAppsPage() to use
// Similar enhancements for other pages: dashboard (fetch usage), api-keys (create/fetch), subscription (fetch status, mock checkout)
